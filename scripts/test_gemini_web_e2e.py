#!/usr/bin/env python3
"""End-to-end test for zero-token Gemini Web provider.

Prerequisites:
  1. Chrome running with: google-chrome --remote-debugging-port=9222
  2. Already logged in to gemini.google.com in that Chrome instance

Usage:
  python scripts/test_gemini_web_e2e.py                  # run all tests
  python scripts/test_gemini_web_e2e.py --test chat      # simple chat only
  python scripts/test_gemini_web_e2e.py --test tool      # tool call only
  python scripts/test_gemini_web_e2e.py --test multi     # multi-turn only
  python scripts/test_gemini_web_e2e.py --cdp http://127.0.0.1:9222
  python scripts/test_gemini_web_e2e.py --model gemini-2.5-pro
"""
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="E2E test for Gemini Web zero-token provider")
    ...
    parser.add_argument("--cookie", default="", help="Google account cookie string (optional if already logged in)")
    ...
    print(f"{CYAN}Gemini Web Zero-Token E2E Test{RESET}")
    print(f"  CDP:   {args.cdp}")
    print(f"  Model: {args.model}")
    print(f"  Note:  Chrome must be open and logged in to gemini.google.com")
